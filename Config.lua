...
CrossHotbar_DB = {
   Config = {
      ...
      SwapType = 2,
      WXHBType = 2,
      FaceButtons = {"PAD1", "PAD2", "PAD3", "PAD4"},
      DPadButtons = {"PADDUP", "PADDRIGHT", "PADDDOWN", "PADDLEFT"},
...
frame:SetScript("OnShow", function(frame)
    ...
    function BindingsToStr(bindings)
       return table.concat(bindings, " ")
    end

    function StrToBindings(text)
       bindings = {}
    ...
    function StorePreset(to, from)
       if to.Mutable then
          ...
          to.SwapBindings = {unpack(from.SwapBindings)}
          to.ExpdBindings = {unpack(from.ExpdBindings)}
       end
    end
    ...
    presetloadbutton:SetScript("OnClick", function(self, button, down)
       StorePreset(config, CrossHotbar_DB.Presets[preset])
       Refresh()
    end)
    ...
    presetsavebutton:SetScript("OnClick", function(self, button, down)
       ...
       elseif CrossHotbar_DB.Presets[foundpreset].Mutable then
          preset = foundpreset
          CrossHotbar_DB.ActivePreset = preset
          StorePreset(CrossHotbar_DB.Presets[preset], config)
       end
       Refresh()
    end)
...
--]]
    ...
    bindingsapply:SetScript("OnClick", function(self, button, down)
       ...
       if #bindings == 4 then
          ...
          config.SwapBindings = {unpack(bindings)}
          config.ExpdBindings = {unpack(bindings)}
       else
